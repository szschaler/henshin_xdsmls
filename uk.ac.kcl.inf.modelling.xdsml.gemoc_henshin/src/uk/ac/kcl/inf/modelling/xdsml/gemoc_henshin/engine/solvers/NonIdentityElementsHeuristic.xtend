...
class NonIdentityElementsHeuristic implements FilteringHeuristic {
	 ...
	 */
	val List<? extends EClassifier> nonIdentityTypes
	
	new (List<? extends EClassifier> nonIdentityTypes) {
	...
	private def equivalentSteps(Step<?> s1, Step<?> s2) {
		...
		if (((hs1.match !== null) && (hs2.match === null)) ||
			((hs2.match !== null) && (hs1.match === null))) {
			return false
		} 
			...
			if (hs1.matches.size != hs2.matches.size) {
				...
				// TODO Need to check for pairwise equality of matches
			}
		} else {
			// TODO Check equality of matches
			equivalentMatches(hs1.match, hs2.match)
		}
	...
	private def equivalentMatches(Match m1, Match m2) {
		(m1 === m2) ||
		(m1.equals(m2) && m2.equals(m1)) || // Doing this both ways because I'm not convinced that Match::equals is actually symmetric
		((m1.rule === m2.rule) &&
			(
